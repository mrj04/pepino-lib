...
import {JasmineExpectElementAttributeNotStrategy} from "./JasmineExpectElementAttributeNotStrategy";

describe("when converting pepino-lang instructions to a jasmine-style expect assertion", () => {
    
    describe("without variables", () => {
        var strategy = new JasmineExpectElementAttributeNotStrategy();
        var instructions = "Verify <#checkboxElement> to not have attribute \"attribute\"";
        ...
        it("should pass on 'have attribute' instructions", () => {
            var similar_instructions = "Verify <#checkboxElement> to have attribute \"attribute\"";    
            expect(strategy.canGenerate(similar_instructions)).to.be.false;    
        });

        it("should convert the assert to jasmine expect code", () => {
            expect(strategy.generate(instructions))
                .to.equal("expect(browser.getHTML(\"#checkboxElement\")).toNotContain(\"attribute\");");
        }); 
