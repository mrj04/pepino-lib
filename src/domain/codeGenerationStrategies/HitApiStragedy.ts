...
export class HitApiStragedy implements ICodeGenerationStrategy {

    canGenerate(text: string): boolean {
        return text.toLowerCase().startsWith("hit api ");
    }

    generate(text: string): string {
        var urls = StringHelper.extractTextInQuotes(text);
        var contents = VariableHelper.getString(urls[0]);
            return `this.Before(function(scenario, callback) {
                         this.apickli = new apickli.Apickli('http', `+ contents + `);
                         callback();
                    });
                ...
                this.When(/^I DELETE without argument (.*)$/, function (resource,  callback) {
                ...
                this.When(/^I DELETE with argument (.*) and (.*)$/, function (resource, argument,  callback) {
                    this.apickli.delete(argument==undefined? resource: resource + this.apickli.getGlobalVariable(argument), function (error, response) {
                        if (error) {
                            callback(new Error(error));
                        }
                ...
                this.Then(/^value of scenario variable (.*) should be (.*)$/, function (variableName, variableValue, callback) {
                    if (this.apickli.assertScenarioVariableValue(variableName, variableValue)) {
                        ...
                        callback(new Error('value of variable ' + variableName + ' is nost equal to ' + variableValue));
                    }
                });
            };
